...
class FragmentMyNews : Fragment() {
    ...
    private fun insertArticles(articleList: MutableList<Article>) {
        checkData(articleList)
        lifecycleScope.launch(Dispatchers.IO) {
            repository.insert(articleList = articleList)
        }
    }

    private fun checkData(articleList: MutableList<Article>) {
        for (i in 0 until articleList.size) {
            if (articleList[i].description == null) articleList[i].description = ""
            if (articleList[i].urlToImage == null) articleList[i].urlToImage = ""
        }
    }
